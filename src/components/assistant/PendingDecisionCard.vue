...
/**
 * 2026-09-18 broker P1：pending 决断卡（一骨架两皮肤，设计真源
 ...
 *  - authz 皮肤（系统授权）：命令原文等宽 + cwd + 规则原文（「你将放行的是什么」）
 *    + 按钮组「允许一次 / 按规则放行（按钮呈现规则原文）/ 拒绝（可附一句话）」。
 *
...
async function decide(action: AuthzDecision): Promise<void> {
  ...
  const record: AuthzDecisionRecord = {
    formId,
    command: view.request.command,
    cwd: view.request.cwd,
    decision: action
  }
  ...
  <div data-test-id="pending-decision-card" :data-kind="decision.kind">
    ...
    >
      ...
      <template v-if="authz.mode === 'pending'">
        <!-- 规则原文（「你将放行的是什么」）：matchedRule 或默认口径派生，按钮同文 -->
