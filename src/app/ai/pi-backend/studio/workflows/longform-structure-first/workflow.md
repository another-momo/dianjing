...
══ polish 段 ══ 全节填完后：describe 全量审计 → look 分区钻取验收 → 按 `references/fix-playbook-structure-first.md` 逐项排查修复。polish 阶段允许调整所有内容（文案重写、布局重排、图片替换、字号/颜色/间距微调），不重发 CP——CP 是与用户确认的节点，polish 在已确认方向内做自由改进。
