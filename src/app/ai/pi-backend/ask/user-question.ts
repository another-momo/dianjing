...
const ASK_USER_QUESTION_DESCRIPTION =
  ...
  ' Kind-decision rules: (a) Set kind multi_select only when multiple answers are valid; otherwise use single_select.' +
  ' (b) single_select options may carry a preview markdown string shown beside the focused option — use it when presenting concrete artifacts the user needs to compare (mockups, layout variants, color schemes, configs, copy versions). Do not use preview for simple preference questions where label+hint suffice. preview is single_select only and never appears in the answer envelope.' +
  ' (c) image_select is for choosing among EXISTING canvas nodes (each option references a real nodeId you obtained from canvas tools) — never create canvas nodes just to make them selectable; for comparing hypothetical options that do not exist on the canvas yet, use single_select with preview instead.'
    ...
    Type.Array(
      Type.Object({
        ...
        preview: Type.Optional(
          Type.String({
            description: 'single_select only: markdown detail shown beside the focused option'
          })
        )
      }),
