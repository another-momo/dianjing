...
describe('FontManager memory governance (T40 S1)', () => {
  ...
  test('protect-via-exclude keeps a just-loaded whale key loaded while older siblings evict', () => {
    // 钉扎②配套：setFontMemoryBudget 在无新键时无 exclude，老键按 LRU 正常卷入；
    ...
    manager.setFontMemoryBudget(400)
    // 600 > 400，setFontMemoryBudget 无 exclude：MemH (100) LRU 先逐，仍需 100 → MemI (500) 也被卷入
  ...
  test('registerAndCache exclude protects the just-loaded whale key', () => {
    // 钉扎②：刚入账键仍经 exclude 豁免——设置低预算后注册超预算键，registerAndCache
    // 传 exclude={该键} 调用 enforce，本键不被卷入；其他老键正常按 LRU 候选
    const manager = new FontManager()
    manager.setFontMemoryBudget(200)
    manager.markLoaded('MemJ', 'Regular', new ArrayBuffer(300))
    // 预算 200、新入账 300：exclude={MemJ} → 本键豁免；总账 300 > 200，留作 overBudgetKeys 诊断快照
