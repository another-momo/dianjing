...
beforeEach(() => {
  ...
  agentDir = join(rootDir, '.dianjing', 'pi-agent')
...
test('T87 listSkills：OFF 时空集（不泄露已扫到 skill 存在性）', () => {
  // 即便 .dianjing/skills 有 SKILL.md，OFF 时 listSkills 也必须空集
  const userSkillsDir = join(rootDir, '.dianjing', 'skills', 'demo')
...
test('T87 listSkills：ON 时扫 cwd/.dianjing/skills（单源）+ 脱敏', () => {
  // T89：单源扫描 .dianjing/skills；原双源去重测试不再适用（同名 demo
  // 在单源下不可能双份；保留 name 投影 + 脱敏两条核心断言）
  const userDir = join(rootDir, '.dianjing', 'skills', 'demo')
...
`,
  ...
  const otherDir = join(rootDir, '.dianjing', 'skills', 'other')
...
test('T87 listSkills：disable-model-invocation 的 skill 也进清单（描述可空兜底）', () => {
  const userDir = join(rootDir, '.dianjing', 'skills', 'hidden')
  mkdirSync(userDir, { recursive: true })
  writeFileSync(
    join(userDir, 'SKILL.md'),
    `---
...
test('T87 listSkills：缺 description → SDK 拒收不进清单（description 是 frontmatter 必填）', () => {
  ...
  const userDir = join(rootDir, '.dianjing', 'skills', 'no-desc')
  mkdirSync(userDir, { recursive: true })
  writeFileSync(
    join(userDir, 'SKILL.md'),
    `---
...
function writeSkill(name: string, body: string): void {
  const dir = join(rootDir, '.dianjing', 'skills', name)
